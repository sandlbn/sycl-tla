#!/bin/bash
# =============================================================================
# CODA Phase 1: K4 + Auxiliary RMS Kernel
#
# K4 GEMM epilogue: D[m,n] = gamma[n] * (acc[m,n] + residual[m,n])
# Aux kernel:       rms_scale[m] = 1/sqrt(mean(D[m,:]^2) + eps)
#
# Usage:
#   sbatch test_phase1.sh
# =============================================================================
#SBATCH --job-name=coda_phase1
#SBATCH --partition=bmtxg31
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=00:30:00
#SBATCH --output=coda_phase1_%j.out

set -e

SYCL_TLA_DIR="/data/nfs_home/mspoczy/upstream/sycl-tla"
BUILD_DIR="/tmp/coda_phase1_build_$$"
mkdir -p "$BUILD_DIR"
...
echo "============================================================"
echo "CODA Phase 1: K4 + Aux RMS Kernel"
echo "============================================================"
echo "Node: $(hostname)"
...
# Compile K4
# ============================================================
echo "Compiling K4 + Aux RMS..."
BINARY="$BUILD_DIR/16_coda_residual_rms"
...
icpx -fsycl \
    ...
    -o "$BINARY" \
    "$SYCL_TLA_DIR/examples/16_coda_residual_rms/16_coda_residual_rms.cpp"
...
# ============================================================
# Correctness (D + RMS scale)
# ============================================================
echo "============================================================"
echo "=== Correctness: K4 + Aux RMS ==="
echo "============================================================"
...
# ============================================================
# Performance: K4+Aux vs bare GEMM
# ============================================================
echo "============================================================"
echo "=== Performance: K4+Aux vs bare GEMM ==="
echo "============================================================"
...
for M in 4096 5120 8192 16384; do
    ...
    echo "--- ${M}x${N}x${K} ---"
    echo "Bare GEMM:"
    "$BASELINE" --m=$M --n=$N --k=$K --iterations=200 --verify=0
    echo "K4 + Aux RMS:"
    "$BINARY" --m=$M --n=$N --k=$K --iterations=200 --verify=0
    echo ""
done
