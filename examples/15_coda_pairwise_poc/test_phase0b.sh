#!/bin/bash
# =============================================================================
# CODA Phase 0B: Pairwise Shuffle Feasibility Study
#
# Tests whether sub-group shuffles in the GEMM epilogue are performant
# enough for CODA pairwise operations (RoPE, SwiGLU).
#
# Usage:
#   sbatch test_phase0b.sh
# =============================================================================
#SBATCH --job-name=coda_phase0b
#SBATCH --partition=bmtxg31
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=00:30:00
#SBATCH --output=coda_phase0b_%j.out

set -e

SYCL_TLA_DIR="/data/nfs_home/mspoczy/upstream/sycl-tla"
BUILD_DIR="/tmp/coda_phase0b_build_$$"
mkdir -p "$BUILD_DIR"
...
echo "============================================================"
echo "CODA Phase 0B: Pairwise Shuffle Feasibility"
echo "============================================================"
echo "Node: $(hostname)"
...
# ============================================================
# Compile pairwise shuffle PoC
# ============================================================
echo "Compiling CODA pairwise shuffle PoC..."
BINARY="$BUILD_DIR/15_coda_pairwise_poc"
...
icpx -fsycl \
    ...
    -o "$BINARY" \
    "$SYCL_TLA_DIR/examples/15_coda_pairwise_poc/15_coda_pairwise_poc.cpp"
...
# ============================================================
echo "============================================================"
echo "=== Correctness: Pairwise swap with verification ==="
echo "============================================================"
...
# ============================================================
# Performance: pairwise swap vs bare GEMM
# ============================================================
echo "============================================================"
echo "=== Performance: GEMM+PairwiseSwap vs bare GEMM ==="
echo "============================================================"
echo "(Overhead target: < 5% of bare GEMM)"
echo ""
...
echo "============================================================"
echo ""
echo "CHECK: Compare TFlop/s numbers above."
echo "  Overhead < 5%  -> Proceed to Phase 2 (production pairwise visitor)"
echo "  Overhead 5-15% -> Proceed cautiously, document break-even"
echo "  Overhead > 15% -> Pivot to dual-GEMM for SwiGLU"
